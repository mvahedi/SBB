    ...
    if not os.path.exists(os.path.dirname(directory)):
        try:
            ...
            if exc.errno != errno.EEXIST:
                raise
    if isTest == True:
        filename = directory + "shuttle_last_generation.csv"
    else:               
        filename = directory + 'shuttle_genration' + str(generation) + '.csv'            
    try:
        fp = open(filename)
    except IOError:
...
def printAllRelevantAttributesForLabels(isTest):
    global attributes_relevant_to_label
    global generation
    directory = os.path.join('results/')
    if not os.path.exists(os.path.dirname(directory)):
        try:
            ...
            if exc.errno != errno.EEXIST:
                raise
    if isTest == True:
        filename = directory + "shuttle_test_attributes.csv"
    else:               
        filename = directory + 'shuttle_attributes' + str(generation) + '.csv'            
    try:
        fp = open(filename)
    except IOError:
        # If not exists, create the file
